...
> The LiDAR Composite DSM (Digital Surface Model) is a raster elevation model covering >60% of England at 1m spatial resolution.
...
I have built three colour palettes that work *ok* with these data, but finding a single colour scheme that works for such diversity of textures is quite challenging. You can add your own using `make.texture()`, which is just a wrapper for Rayshader's `create_texture()` that also names it and saves an example.
...
An 'rgl' window will now open. The 'rgl' device is interactive: you can rotate by clicking and dragging with your left mouse button, control zoom with the centre button and change the simulated focal length with the right button. Have fun!

<br>

### Aerial photography!
We can grab a couple of snapshots from different angles before closing the 'rgl' window.
I have predefined some simulated cameras (e.g. `angle_1` below), which can be found in the source code. Feel free to add your own!
...
Now, let's render the same scene with different lighting: a lower 'sun' shining from the ENE. You might have noticed that, in the previous images, the hills looked taller than they actually are - their real height is a bit, ahem, underwhelming. This time, let's take them down to their real height by setting `exaggerate_z` to `FALSE`. I included this argument that makes everything 40% taller beacuse, well, why not.
